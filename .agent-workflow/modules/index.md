...
|--------|------|:----:|---------|:----:|:----:|:--------:|--------|----------|
| desktop-surface | desktop surface（Electron 桌面外壳，无后台服务） | MODULE | `dsh --profile dsh-desktop` 起 Electron 原生窗口加载 DSH web（无本地 HTTP 监听，Remote API + IPC 桥接），零定制复用；交互式终端下自动转交 detached 子进程让命令立即返回；支持打包 Electron 运行时发 Release | 🟢 已完成 | 🟢 有效 | 2026-09-19 | desktop,electron,surface,detach,background,web,ipc,profile | [desktop-surface.md](./desktop-surface.md) |
